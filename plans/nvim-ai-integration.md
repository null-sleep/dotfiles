...
starts then immediately exits with `code=1`. Happens in `trade-services` but not `dotfiles`.
...
- [ ] Re-add plugin and verify fix works reliably in trade-services
...
- [ ] Investigate root cause of ACP init failure in trade-services
