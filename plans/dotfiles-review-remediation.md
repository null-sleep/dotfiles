...
`~/.zshrc_work.zsh` (README:932) so committing the ignore file breaks nothing,
this machine has no work symlink and no loaded yknotify agent.
...
- **C3 [endorsed]** `.zshrc_work.zsh:53`: `alias bga='$HOME/src/admin-tools/bin/admintool'`.
- **C4 [endorsed]** `.zshrc_work.zsh:28,50`: gate `colima_check_and_start` +
  `yknotify_check` behind `/tmp/.work-shell-checks-$UID` stamp (per-boot;
...
- **C8 [revised — verify first]** `.zshrc_shared.zsh:7` mise double-activation:
...
- README: delete the `sed __USER__` block + `launchctl load/start`; rewrite
  section (see E4). `.zshrc_work.zsh` `yknotify_check` → new label + script.
Rejected alternative: minimal username re-substitution — leaves deprecated
load, no throttle, breaks again on next machine.

### C1. Work fail-open stow [endorsed]
Commit `zsh/.stow-local-ignore` (content `.zshrc_work.zsh`); remove line 1 of
`.gitignore`. Safe: work machines drop a *real* file (README:932), never rely on
the stow symlink; verified this machine has no `~/.zshrc_work.zsh` at all.
...
12. `fix(zsh): work $HOME alias, per-boot check gate, mise dedupe` (C3+C4+C8)
...
- stow: `stow -R zsh claude nvim yknotify macos` → no new unexpected symlinks in
  `$HOME` (specifically no `~/.zshrc_work.zsh`, no `~/yknotify.sh`).
