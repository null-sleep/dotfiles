...
-- Gate image rendering to frontends that can display one: snacks' doc path
-- fails OPEN, so ungated it emits kitty-graphics escapes into iTerm2/Neovide.
-- Key off terminal env vars, not $TERM (tmux rewrites it; Neovide inherits
-- xterm-ghostty) — these survive into multiplexer panes, so tmux still works.
local function can_render_images()
  if vim.g.neovide then return false end             -- GUI, no image protocol
  if os.getenv('ZELLIJ') then return false end       -- drops graphics escapes
  return os.getenv('GHOSTTY_RESOURCES_DIR') ~= nil   -- ghostty
      or os.getenv('KITTY_WINDOW_ID') ~= nil         -- kitty
      or os.getenv('WEZTERM_PANE') ~= nil            -- wezterm
end

-- snacks detects Ghostty only via an async probe with no env fallback, and
-- memoizes it; lose that race and it caches placeholders=false, dropping the doc
-- path to hover mode so diagrams vanish on buffer switch. Force it on.
...
require('snacks').setup({
  ...
  image = {
    ...
    math = { enabled = false },
    doc = { conceal = false }, -- keep diagrams visible while the cursor is on them
  },
