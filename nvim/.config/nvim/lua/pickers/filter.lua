-- pickers/filter.lua — Telescope picker for toggling file-type filter presets.
--
-- USAGE
--   require('pickers.filter').pick()   open the picker
--   require('pickers.filter').get()    returns { names, globs } of enabled presets, or nil
--
-- PRESETS
...
function M.live_grep()
  local f = M.get()
  if f then
    builtin.live_grep({ glob_pattern = f.globs })
  else
    builtin.live_grep()
  end
end
