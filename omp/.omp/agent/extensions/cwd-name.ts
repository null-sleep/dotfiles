...
type CtxLike = { worktree?: { worktreeName?: string } };

/**
 * Launch-folder segment for the native status line, so a shell-launched omp
 * shows which project it's in: the basename of the working directory, or the
 * worktree basename when the segment context identifies a linked worktree.
 * Registered through the live `SEGMENTS` record like turn-count.ts (which
 * documents the mechanism and theme/render caveats followed here). Inside an
 * nvim sidekick terminal ($NVIM inherited) nvim already shows the project, so
 * skip registration — the configured id then renders invisible.
 */
    ...
    registry[SEGMENT_ID] = {
      ...
      render: (ctx) => {
        try {
          ...
          const name = wt?.worktreeName || path.basename(process.cwd());
