...
`nvim-notify.ts`, `turn-count.ts`, `cwd-name.ts`, `cycle-model-picker.ts`, and
`openrouter-session-cost.ts` extensions — config.yml is seeded via `omp config`
and stays machine-local, and `~/.omp/agent/` also holds runtime state, so it's
...
`nvim/.config/nvim/AGENTS.md` for how this repo maintains `GUIDE.md` (the
...
- **Part 2: Reference** — one `## ` section per tool/app, grouped by category
  in the `## Contents` TOC (AI & Codex tooling, Editors, Terminals &
...
Same rule as the nested nvim `AGENTS.md`: headings with parentheses, dots,
